import type { StatsSummary, Race, Horse, Prediction, Bet, Result, RaceCreate, RaceUpdate, HorseCreate, HorseUpdate, PredictionCreate, PredictionUpdate, BetCreate, BetUpdate, ResultCreate, ResultUpdate, Venue, BetType } from '../types';
...
export const updateRace = async (raceId: number, data: RaceUpdate): Promise<Race> => {
...
export const updatePrediction = async (predictionId: number, data: PredictionUpdate): Promise<Prediction> => {
...
export const updateBet = async (betId: number, data: BetUpdate): Promise<Bet> => {
  ...
  if (!response.ok) {
    throw new Error('馬券の更新に失敗しました');
  }
  return response.json();
};

export const deleteBet = async (betId: number): Promise<void> => {
  const response = await fetch(`/api/v1/bets/${betId}`, {
    method: 'DELETE',
  });
  if (!response.ok) {
    throw new Error('馬券の削除に失敗しました');
  }
};

export const updateResult = async (resultId: number, data: ResultUpdate): Promise<Result> => {
...
export const deleteResult = async (resultId: number): Promise<void> => {
  ...
  if (!response.ok) {
    throw new Error('レース結果の削除に失敗しました');
  }
};
