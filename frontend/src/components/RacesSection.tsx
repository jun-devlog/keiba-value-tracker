              ...
              <tbody>
                {races.map((race) => {
                  if (editingRaceId === race.id) {
                    return (
                      <tr key={race.id} className="inline-edit-row">
                        <td colSpan={6}>
                          <RaceUpdateForm
                            race={race}
                            onSave={async (data) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        <span className="td-bold">{race.race_name}</span>
                        {race.grade && <span className="badge-grade">{race.grade}</span>}
                      </td>
                      <td className="td-dim">{race.track_type} {race.distance}m</td>
                      <td>
                        <button
